...
function OrderDetail() {
  ...
  // Pseudo-progress: order is always at "out_for_delivery" simulated step
  const currentIndex = order.status === "delivered" ? 3 : order.status === "out_for_delivery" ? 2 : order.status === "packing" ? 1 : 0;
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 md:px-6 md:py-10">
        ...
        <div className="mt-4 rounded-3xl border border-border/60 bg-card p-5 shadow-card md:p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-success/10 px-3 py-1 text-[12px] font-semibold text-success">{order.status.replaceAll("_", " ")}</span>
          </div>
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <li key={s.key} className="flex flex-col items-center text-center">
                  <div className={cn("grid h-9 w-9 place-items-center rounded-full border-2", done ? "border-primary bg-primary text-primary-foreground" : "border-border bg-surface text-muted-foreground")}>
                    {done ? <Icon className="h-4 w-4" /> : <CircleDot className="h-4 w-4" />}
                  </div>
                  <span className={cn("mt-2 text-[11px]", done ? "font-semibold text-foreground" : "text-muted-foreground")}>{s.label}</span>
                </li>
              );
            })}
          </ol>
          <div className="relative mt-3 h-1 rounded-full bg-border">
            <div className="absolute inset-y-0 left-0 rounded-full bg-primary transition-all" style={{ width: `${(currentIndex / 3) * 100}%` }} />
          </div>
          <p className="mt-3 text-center text-[12.5px] text-muted-foreground">Estimated delivery within {order.slot}</p>
        </div>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-[1fr_280px]">
            ...
            <ul className="mt-3 space-y-3">
              {order.items.map((it, i) => (
                <li key={i} className="flex items-center gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl text-2xl" style={{ background: it.imageBg }}>{it.imageEmoji}</span>
          ...
          <aside className="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between text-muted-foreground"><dt>Subtotal</dt><dd className="text-foreground">{formatCurrency(order.subtotal)}</dd></div>
              <div className="flex justify-between text-muted-foreground"><dt>Delivery</dt><dd className="text-foreground">{order.delivery ? formatCurrency(order.delivery) : "Free"}</dd></div>
              <div className="flex justify-between border-t border-border pt-2 font-semibold"><dt>Total</dt><dd>{formatCurrency(order.total)}</dd></div>
            </dl>
